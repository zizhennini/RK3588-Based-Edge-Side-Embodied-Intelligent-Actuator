"""语音助手 CLI — voice/ 包统一命令行入口

从旧版 voice_assistant/voice_assistant/cli.py 迁移（refactor_plan_v9 T1.6），
适配扁平 voice/ 包结构，并新增 --no-photo 纯文字问答开关。

Usage:
    python va.py listen-forever --wake-mode kws   # 持续唤醒监听
    python va.py ask "介绍一下你的功能"            # 文字问答（意图可触发拍照）
    python va.py ask "你好" --no-photo            # 纯文字问答（禁用拍照）
    python va.py once --seconds 4                 # 单轮语音识别
"""
...
def _make_motion_cb() -> "callable":
    """构造动作库回放回调 — 匹配到动作时后台启动 scripts/replay_traj.py"""

    def motion_cb(action: str, info: dict) -> None:
        traj = _PROJECT_ROOT / "motion_library" / info["file"]
        if not traj.is_file():
            print(f"[动作] 轨迹文件不存在: {traj}", flush=True)
            return
        cmd = [sys.executable, str(_PROJECT_ROOT / "scripts" / "replay_traj.py"),
               str(traj), "--port", "/dev/ttyACM0", "--fps", "30", "--initial"]
        print(f"[执行] {' '.join(cmd)}", flush=True)
        subprocess.Popen(cmd, start_new_session=True, cwd=str(_PROJECT_ROOT),
...
def main() -> None:
    ...
    assistant = VoiceAssistant(cfg, motion_cb=_make_motion_cb())
...
def _run_command(args: argparse.Namespace, assistant: VoiceAssistant) -> None:
    no_photo = bool(getattr(args, "no_photo", False))
    if args.cmd == "record":
